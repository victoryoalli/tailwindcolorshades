  ...
  methods: {
    ...
    addColor(e) {
      ...
      if (!this.newColor.name) {
        this.newColor.name = colorNamer(
          `#${this.newColor.color}`.replace("##", "#")
        ).ntc[0].name;
        this.$gaTrack(
          "colors",
          "color:generation",
          `No name provided for color: ${this.newColor.color}). Using the automated name: ${this.newColor.name}`
        );
      }
...
<template>
  <div class="new-color">
    <form @submit.prevent="addColor" class="flex flex-wrap justify-around flex-1 mx-auto">
      ...
      <div class="flex flex-col flex-1 mr-2">
        <label class="block font-bold text-left">Color name</label>
        <input
          type="text"
          class="input orange bg-grey-lighter"
